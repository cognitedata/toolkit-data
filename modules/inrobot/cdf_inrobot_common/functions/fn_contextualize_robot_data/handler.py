...
def get_timeseries_external_id(
    client, asset, data_set_external_id, time_series_label, number_of_gauges=1
):
    """Get time series external id."""
    ts = client.time_series.list(
        metadata={"TYPE": time_series_label}, asset_ids=[asset.id]
    )
    ...
    # Logic for creating gauge timeseries
        ...
        if number_of_gauges <= 1:
            ts = create_timeseries(
                client, asset, data_set_external_id, time_series_label
            )
            ts_external_ids.append(ts.external_id)
        else:
...
def handle(data, client):
    ...
    if not required_input_data_fields <= data.keys():
        raise RuntimeError(
            f"Data should contain all keys:  {required_input_data_fields}. Current data: {data}"
        )
    ...
    logger.info(
        f"Number of files to process in dataset  {data['data_set_external_id']}: {len(files)}"
    )
    print(
        f"Number of files to process in dataset  {data['data_set_external_id']}: {len(files)}"
    )
    ...
    for file in files:
        ...
        if file.labels and has_label(file.labels, data["gauge_context_label"]):
            gauge_type = metadata.get(
                "gauge_type", "dial"
            )  # Default gauge type from robot is dial
                        ...
                        for gauge_number in range(number_of_gauges):
                            if metadata.get("ts_external_id", None):
                                if gauge_number == 0:
                                    ...
                                    metadata[f"ts_external_id_{gauge_number}"] = ts_eid[
                                        gauge_number
                                    ]
                            else:
                                metadata[f"ts_external_id_{gauge_number}"] = ts_eid[
